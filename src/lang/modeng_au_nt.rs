...
pub fn engaudarwin(original_text: &str, usefile: &str, outputfile: &str, reset: &str, red: &str, cyan: &str, yellow: &str) {
    ...
    if usefile == "new" {
        ...
        file.write_all("DARWIN, NT, AU:".as_bytes()).expect(&(red.to_owned() + "write failed" + reset));
    ...
    if usefile == "terminal" {
        println!();
        println!("{}", cyan.to_owned() + "Darwin" + reset + ", " + cyan + "NT" + reset + ", " + cyan + "AU" + reset + ":");
        println!();
        print!("{yellow}");
        println!("{result}");
        print!("{reset}");
